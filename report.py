...
表格欄位（依序）:
  ...
  2. Affected IHS Version（來自內頁 Affected Products and Versions）
  3. CVE-ID（連結至 MITRE）
  4. Severity（顏色 badge）
  5. Publish Date
  6. CVSS Base Score（顏色標籤）
  7. iFix（V9 / V8.5 各自連結）
  8. Fixpack Version（V9 / V8.5）
  9. Fixpack Release Date（V9 / V8.5）
"""
...
def _ifix_cell(
    ...
    lts = _make_link(lts_label, lts_url, "V9: ")
    cd = _make_link(cd_label, cd_url, "V8.5: ")
    if lts:
        lines.append(lts)
    if cd:
        lines.append(cd)
...
def _fixpack_cell(lts: str, cd: str) -> str:
    """
    回傳 Fixpack Version 欄位 HTML（V9 / V8.5）。
    lts（V9）/ cd（V8.5）為多行字串（\n 分隔），每行一個版本。
    """
    items = []
    for ver in lts.split("\n"):
        ver = ver.strip()
        if ver:
            items.append(f'<span class="version-tag lts-tag">V9: {ver}</span>')
    for ver in cd.split("\n"):
        ver = ver.strip()
        if ver:
            items.append(f'<span class="version-tag cd-tag">V8.5: {ver}</span>')
    return "<br>".join(items) if items else '<span class="text-muted">—</span>'


def _fixdate_cell(lts_date: str, cd_date: str) -> str:
    """
    回傳 Fixpack Release Date 欄位 HTML（V9 / V8.5）。
    lts_date（V9）/ cd_date（V8.5）為多行字串（\n 分隔），與對應的 fixpack_lts/cd 行數一致。
    各行加上 V9:/V8.5: 前綴與顏色標記，方便與 Fixpack Version 欄對照。
    """
    ...
    if has_lts:
        for d in lts_date.split("\n"):
            d = d.strip()
            if d:
                result_items.append(f'<span class="date-tag lts-date-tag">V9: {d}</span>')
            else:
                result_items.append('<span class="text-muted">V9: —</span>')
    if has_cd:
        for d in cd_date.split("\n"):
            d = d.strip()
            if d:
                result_items.append(f'<span class="date-tag cd-date-tag">V8.5: {d}</span>')
            else:
                result_items.append('<span class="text-muted">V8.5: —</span>')
...
<head>
  ...
  <title>IBM HTTP Server High-Risk CVE Report</title>
...
<div class="page-wrapper">
  ...
  <div class="report-header">
    <h1>🔒 IBM HTTP Server — High-Risk CVE Report</h1>
    <div class="subtitle">
      ...
      <table id="cveTable" class="table table-bordered table-sm align-middle w-100">
        <thead>
          <tr>
            ...
            <th class="text-center">Affected IHS Version</th>
        ...
        <tbody>
{table_rows if table_rows else '          <tr><td colspan="9" class="text-center text-muted py-4">沒有找到符合條件的 IBM HTTP Server 高風險 CVE 公告</td></tr>'}
        </tbody>
      </table>
    </div>
  </div>
  ...
  <div class="report-footer">
    Generated by IBM HTTP Server CVE Scraper &nbsp;|&nbsp; {generated_time}
  </div>
